...
namespace Drupal\Tests\dkan_dataset\Functional;
 ...
 * @group node
 */
class DatasetCreationTest extends NodeTestBase {
   ...
   * @var array
   */
  public static $modules = ['node_test_exception', 'dblog', 'test_page_test', 'dkan_dataset'];

  protected function setUp() {
    parent::setUp();

    $web_user = $this->drupalCreateUser(['create dataset content', 'edit own dataset content']);
   ...
   */
  public function testDatasetCreation() {
    $node_type_storage = \Drupal::entityManager()->getStorage('node_type');

    // Test /node/add page with only one content type.
    $node_type_storage->load('article')->delete();
    $node_type_storage->load('page')->delete();
    $this->drupalGet('node/add');
    $this->assertResponse(200);
    $this->assertUrl('node/add/dataset');
    // Create a node.
    $edit = [];
    $edit['title[0][value]'] = $this->randomMachineName(8);
    $edit['body[0][value]'] = $this->randomMachineName(16);
    $this->drupalPostForm('node/add/dataset', $edit, t('Save'));

    // Check that the Basic page has been created.
    $this->assertText(t('@post @title has been created.', ['@post' => 'Basic dataset', '@title' => $edit['title[0][value]']]), 'Basic dataset created.');
    ...
    $admin_user = $this->drupalCreateUser(['administer nodes', 'create dataset content']);
  ...
  public function testUnpublishedNodeCreation() {
    ...
    $this->assertUrl('');
    $this->assertText(t('Test dataset text'));

    // Confirm that the node was created.
    $this->assertText(t('@post @title has been created.', ['@post' => 'Basic dataset', '@title' => $edit['title[0][value]']]));
  ...
  public function testAuthorAutocomplete() {
    ...
    $admin_user = $this->drupalCreateUser(['administer nodes', 'create dataset content', 'access user profiles']);
    $this->drupalLogin($admin_user);
